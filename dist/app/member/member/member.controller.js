"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MemberController = void 0;
const common_1 = require("@nestjs/common");
const member_service_1 = require("./member.service");
const member_model_1 = require("./member.model");
const swagger_1 = require("@nestjs/swagger");
let MemberController = class MemberController {
    memberService;
    constructor(memberService) {
        this.memberService = memberService;
    }
    async register(memberbody) {
        const existingMember = await this.memberService.getMemberByEmail(memberbody.email);
        if (existingMember) {
            throw new common_1.BadRequestException(`Member with email ${memberbody.email} already exists.`);
        }
        return this.memberService.registerMember(memberbody.name, memberbody.email, memberbody.startdate);
    }
    async getMember(email) {
        const member = await this.memberService.getMemberByEmail(email);
        if (!member) {
            throw new common_1.BadRequestException(`No member found with email ${email}.`);
        }
        return member;
    }
    async getAllActiveMembers() {
        return await this.memberService.getAllActiveMembers();
    }
    async cancel(email) {
        const member = await this.memberService.getMemberByEmail(email);
        if (!member) {
            throw new common_1.BadRequestException(`No member found with email ${email}.`);
        }
        return await this.memberService.cancelMembership(email);
    }
    async modify(memberbody) {
        const member = await this.memberService.getMemberByEmail(memberbody.email);
        if (!member) {
            throw new common_1.BadRequestException(`No member found with email ${memberbody.email}.`);
        }
        return await this.memberService.modifyStartDate(memberbody.email, memberbody.newstartdate);
    }
};
exports.MemberController = MemberController;
__decorate([
    (0, common_1.Post)('register'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [member_model_1.registermember]),
    __metadata("design:returntype", Promise)
], MemberController.prototype, "register", null);
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiQuery)({ name: 'email', required: true }),
    __param(0, (0, common_1.Query)('email')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], MemberController.prototype, "getMember", null);
__decorate([
    (0, common_1.Get)('active'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], MemberController.prototype, "getAllActiveMembers", null);
__decorate([
    (0, common_1.Delete)('cancel'),
    (0, swagger_1.ApiQuery)({ name: 'email', required: true }),
    __param(0, (0, common_1.Query)('email')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], MemberController.prototype, "cancel", null);
__decorate([
    (0, common_1.Put)('update'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [member_model_1.updatememberreq]),
    __metadata("design:returntype", Promise)
], MemberController.prototype, "modify", null);
exports.MemberController = MemberController = __decorate([
    (0, common_1.Controller)('members'),
    __metadata("design:paramtypes", [member_service_1.MemberService])
], MemberController);
//# sourceMappingURL=member.controller.js.map